 ...
 * deploy-pool-v10.mjs
 *
 * Deploys shielded_pool v10, which adds a Tier-C on-chain KYC gate: deposit() and
 * withdraw() now perform a cross-contract get_attestation() call into AttestProtocol
 * and require the transacting wallet to hold a valid, non-revoked, unexpired KYC
 * attestation issued by the trusted authority under the configured schema.
 *
 * What changed vs v9:
 *   - Constructor takes 3 extra args: --attest_protocol, --kyc_authority, --kyc_schema.
 *   - deposit/withdraw take an extra --kyc_attestation_uid (32-byte hex) at the end.
 *     (transfer is unchanged — it moves value privately between already-onboarded
 *      parties, so the KYC edges are deposit/withdraw, per PRD §6.2.)
 *
 * Prereqs (in order):
 ...
 *
 * Usage:
 *   node scripts/deploy-pool-v10.mjs            # deploy only
 *   RUN_SELFTEST=1 node scripts/deploy-pool-v10.mjs   # deploy + full corridor self-test
 *
 * Rollback: v9 remains deployed; testnet-addresses.json keeps the previous pool id
 * under contracts.shielded_pool_v9_prev. Point the frontend back to it to revert.
 */
...
async function main() {
  log("╔═══════════════════════════════════════════════════════════╗");
  log("║  ShadowWire — Deploy Pool v10 (on-chain KYC attestations)  ║");
  log("╚═══════════════════════════════════════════════════════════╝");
  ...
  section("Step 2 — Deploy pool v10 (reusing verifiers + registry + AttestProtocol)");
  ...
  const POOL_V10 = extractContractId(deployOut);
  if (!POOL_V10) throw new Error("Failed to extract pool contract ID from deploy output");
  log(`  Pool v10 deployed: ${POOL_V10}`);

  const prevPool = addrs.contracts.shielded_pool;
  // Preserve the original v9 rollback target; track the previous v10 separately
  // so re-running this script never clobbers the v9 fallback.
  if (!addrs.contracts.shielded_pool_v9_prev) addrs.contracts.shielded_pool_v9_prev = prevPool;
  addrs.contracts.shielded_pool_v10_prev = prevPool;
  addrs.contracts.shielded_pool = POOL_V10;
  addrs.txs = addrs.txs ?? {};
  addrs.txs.pool_v10_deploy = extractTx(deployOut) ?? "";
  saveAddresses(addrs);
  log(`  testnet-addresses.json updated — active pool: ${POOL_V10}`);

  if (!RUN_SELFTEST) {
    section("Step 3 — Corridor self-test skipped (set RUN_SELFTEST=1)");
    log(`  ✅ Pool v10 deployed with on-chain KYC attestation gate.`);
    return;
  }
  ...
  const depositOut = stellar(
    "contract", "invoke", "--id", POOL_V10, "--network", NETWORK,
  ...
  const expectedRoot = depositNote.newRoot.replace(/^0x/, "");
  {
    ...
    while (Date.now() < deadline) {
      try {
        const rootOut = stellar("contract", "invoke", "--id", POOL_V10, "--network", NETWORK,
  ...
  finalAddrs.txs.corridor_deposit_v10  = extractTx(depositOut)  ?? "";
  finalAddrs.txs.corridor_transfer_v10 = extractTx(transferOut) ?? "";
  finalAddrs.txs.corridor_withdraw_v10 = extractTx(withdrawOut) ?? "";
  finalAddrs.txs.kyc_attest_alice = aliceUid;
  finalAddrs.txs.kyc_attest_bob   = bobUid;
  saveAddresses(finalAddrs);

  log("\n╔═══════════════════════════════════════════════════════════╗");
  log("║  Pool v10 deployed; KYC-gated corridor verified ✅         ║");
  log("╚═══════════════════════════════════════════════════════════╝");
}

main().catch(err => {
  console.error("\n❌  deploy-pool-v10 failed:", err.message ?? err);
