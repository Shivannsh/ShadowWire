...
1. **On-chain KYC attestation (AttestProtocol, Tier C).** The mock-issuer acts as a real KYC *authority*: it issues **delegated** attestations (signed off-chain with a BLS12-381 key registered on-chain) about a wallet under a registered KYC schema. On `deposit()` and `withdraw()` the `shielded_pool` performs a **cross-contract `get_attestation()` call into AttestProtocol** and enforces every binding: the attestation exists, its `attester` is the trusted authority, its `schema_uid` matches, its `subject` is the authenticated caller, it is not revoked, it has not expired, **and its `value` (the actual KYC claim — `verified`/`tier`/`country`) matches the policy required for that corridor edge** (sending edge on `deposit()`, receiving edge on `withdraw()`). The pool no longer just checks that an attestation exists — it enforces the claim. A user cannot present someone else's attestation (subject is bound to the caller), a self-issued one is rejected (attester is pinned), and an attestation for the wrong jurisdiction/tier is rejected (`KycAttestationValueMismatch`).
   - **Trust boundary (honest).** The single place where "did this wallet pass KYC?" is decided is the issuer's `runKycCheck()` seam (`mock-issuer/src/kyc-attest.ts`) — today it does sanctions-denylist screening + input validation and is where a real provider (Sumsub/Onfido/SEP-12 anchor) plugs in. The pool trusts the *credential*, not the process; swapping the seam is a code-local change that never touches the contract.
